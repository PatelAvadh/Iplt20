...
public class BatchConfig {
	...
	@Bean
	public FlatFileItemReader<MatchDataIngestion> reader() {
		return new FlatFileItemReaderBuilder<MatchDataIngestion>()
				.name("MatchItemReader")
				.resource(new ClassPathResource("match-data.csv"))
				.delimited()
				.names(FIELD_NAMES)
				.fieldSetMapper(new BeanWrapperFieldSetMapper<MatchDataIngestion>() {{
					setTargetType(MatchDataIngestion.class);
				}})
				.build();
	}
	...
	public Step step1(JdbcBatchItemWriter<Match> writer) {
	  return stepBuilderFactory.get("step1")
	    .<MatchDataIngestion, Match> chunk(10)
	    .reader(reader())
	    .processor(processor())
	    .writer(writer)
	    .build();
